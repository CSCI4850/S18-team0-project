...
Our project deals with document authorship attribution. It is specifically aimed at determining who wrote small documents, such as reviews, emails, texts, and other similar media given a set of small text with labeled authors. Smaller text requires a certain finesse and is quite a challenging classification task. Neural networks, by nature,  are adept at learning sophisticated patterns unseen by human perception. We would like  to utilize them in such a way to better determine authorship of these documents. Using Doc2Vec and our own custom encoding, we hope to be able produce accurate attribution when determining who wrote an individual small document. Our own method uses pre-trained Word2Vec vectors from two sources, Google's pre-trained news vectors and spaCy's en_core_web_lg. We calculate a vector representation of the document through the summation of all individual word vectors in the document.

Primarily, three neural networrks are used and they are small in size. For training and testing, we normally set the number of authors to either 3 or 35.
